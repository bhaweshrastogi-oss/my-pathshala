...
const BACKEND_URL = 'https://my-pathshala-3i17b81mj-bhaweshrastogi-oss-projects.vercel.app';
...
async function initiatePhonePeRedirect() {
  ...
  try {
    ...
    if (data.redirectUrl) {
      ...
      // ── Open PhonePe PayPage using official checkout.js ──────────
      // PhonePeCheckout.transact() is provided by:
      // <script src="https://mercury.phonepe.com/web/bundle/checkout.js">
      // which is loaded in index.html BEFORE this file.
      if (window.PhonePeCheckout && window.PhonePeCheckout.transact) {
        window.PhonePeCheckout.transact({
          tokenUrl : data.redirectUrl,
          type     : 'REDIRECT', // Opens PhonePe in same tab; use 'IFRAME' to embed
          callback : function(response) {
            if (response === 'CONCLUDED') {
              // Payment done — show success screen
              showPaySuccess();
              notifyPaymentConfirmed(orderRef, name, email, course, timestamp).catch(() => {});
              triggerConfirmationEmail(name, email, course, orderRef).catch(() => {});
        ...
        // Fallback: checkout.js didn't load — do direct redirect
        console.warn('[PhonePe] checkout.js not loaded, falling back to redirect');
...
// Called by PhonePeCheckout callback when response === 'CONCLUDED'
function showPaySuccess() {
  confirmPaymentSuccess();
}

// Notify admin via Web3Forms that payment was initiated
async function notifyPaymentConfirmed(orderRef, name, email, course, timestamp) {
  await fetch('https://api.web3forms.com/submit', {
    method : 'POST',
    headers: { 'Content-Type': 'application/json' },
    body   : JSON.stringify({
...
// Trigger student confirmation email via backend
async function triggerConfirmationEmail(name, email, course, orderRef) {
